 ...
 */
package com.race604.fetion.client.dispatcher;
...
import com.race604.fetion.client.NotifyHandler;
import com.race604.fetion.client.ResponseHandler;
 ...
 * @author solosky <solosky772@qq.com>
 */
public abstract class AbstractMessageDispatcher extends AbstractProcessor
	 ...
	 *            当前duihk
	 */
	public AbstractMessageDispatcher(FetionContext client, Dialog dialog,
	...
	protected Object doProcessIncoming(Object o) throws FetionException {
		try {
			...
			if (in instanceof SipcNotify) {
				...
				throw new DispatcherException("Unkown SipMessage type.."
						+ in.getClass().getName());
			}
	...
	public void dispatch(SipcResponse response) throws FetionException {
		...
		// 检查回复对应的请求
		if (request == null) {
			// logger.warn("Request not found in response - response=" +
			// response);
			return;
		}
		...
		if (handler == null) {
			// logger.warn("ResponseHandler not found in request - response=" +
			// response);
			return;
		}
	...
	public void dispatch(SipcNotify notify) throws FetionException {
		...
		if (clazz == null) {
			throw new DispatcherException(
					"Cannot find a class to handle this notify - notify:\n"
							+ notify.toSendString());
		}
		// 加载这个类
	 ...
	 * @return
	 */
	private NotifyHandler loadNotifyHandler(String clazz)
			throws DispatcherException {
		NotifyHandler handler = null;
		handler = this.notifyHandlers.get(clazz);
		if (handler == null) {
			try {
				handler = (NotifyHandler) Class.forName(clazz).newInstance();
