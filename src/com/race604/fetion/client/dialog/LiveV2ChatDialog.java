...
import com.race604.fetion.client.dispatcher.LiveV2MessageDispatcher;
...
import com.race604.fetion.net.Port;
import com.race604.fetion.net.RequestTimeoutException;
import com.race604.fetion.net.Transfer;
...
import com.race604.fetion.sipc.SipcNotify;
...
import com.race604.fetion.util.BuddyEnterHelper;
import com.race604.fetion.util.TicketHelper;
 ...
 * @author solosky <solosky772@qq.com>
 */
public class LiveV2ChatDialog extends ChatDialog implements MutipartyDialog,
		ExceptionHandler {
	...
	private void buildProcessorChain(Transfer transfer) throws FetionException {
		...
		this.processorChain.addLast(new LiveV2MessageDispatcher(context, this,
				this)); // 消息分发服务
		if (FetionConfig.getBoolean("log.sipc.enable"))
			this.processorChain.addLast(new SipcLogger("LiveV2ChatDialog-"
					+ mainBuddy.getFetionId())); // 日志记录
	...
	public void sendChatMessage(Message message, ActionEventListener listener) {
		this.ensureOpened();
		SipcRequest request = this.messageFactory.createSendChatMessageRequest(
				this.mainBuddy.getUri(), message);
	...
	protected void doOpenDialog() throws Exception {
		...
		if (this.isBeenInvited()) {
			// 如果被邀请，进入凭证是在邀请通知里
			ticket = this.inviteNotify.getHeader(SipcHeader.AUTHORIZATION)
					.getValue();
		} else {
			// 如果不是，用户主动发起会话请求，需要向服务器获取进入凭证
			ticket = this.context.getDialogFactory().getServerDialog()
					.startChat();
		}
		...
		// 等待好友进入对话框
		int waitTimeout = FetionConfig
				.getInteger("fetion.dialog.wait-buddy-enter-timeout");
		this.buddyEnterHelper
				.waitBuddyEnter(this.mainBuddy, waitTimeout * 1000);
	 ...
	 * @throws TransferException
	 */
	private void register(String credential) throws RequestTimeoutException,
			InterruptedException, IllegalResponseException, TransferException {
		// 发送注册信息
		SipcRequest request = this.messageFactory
				.createRegisterChatRequest(credential);
	 ...
	 */
	private void invite() throws IllegalResponseException,
			RequestTimeoutException, InterruptedException, TransferException,
			SystemException {
		ActionEventFuture future = new ActionEventFuture();
		this.inviteBuddy(this.mainBuddy, new FutureActionEventListener(future));
		assertActionEvent(future.waitActionEventWithException(),
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.client.dialog.MutipartyDialog#getBuddyByUriList()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.client.dialog.MutipartyDialog#inviteBuddy(net
	 * .solosky.maplefetion.bean.FetionBuddy,
	 * net.solosky.maplefetion.client.dialog.ActionEventListener)
	 */
	@Override
	public void inviteBuddy(Buddy buddy, ActionEventListener listener) {
		SipcRequest request = this.messageFactory
				.createInvateBuddyRequest(this.mainBuddy.getUri());
		request.setResponseHandler(new DefaultResponseHandler(listener));
		this.process(request);
	}

	/**
	 * 异常回调函数 注意：这里的异常不是在调用者调用方法的时候发生的，而是在传输对象读取一个接受消息时产生的异常，是由客户端内部产生的
	...
	public void handleException(FetionException e) {
		...
		if (e instanceof TransferException) {
			...
			// 记录这个错误
			// logger.warn("LiveV2ChatDialog exception caught, just ignore it..",
			// e);
		}
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.client.dialog.MutipartyDialog#buddyEntered(net
	 * .solosky.maplefetion.bean.Buddy)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.client.dialog.MutipartyDialog#buddyLeft(net.solosky
	...
	public String toString() {
		return "[LiveV2ChatDialog - " + "MainBuddy="
				+ mainBuddy.getDisplayName() + ", " + mainBuddy.getUri() + " ]";
	}
}
