...
import com.race604.fetion.net.AutoTransferFactory;
...
public class FetionClient implements FetionContext {
	 ...
	 * MapleFetion版本
	 */
	public static final String CLIENT_VERSION = "SMS-0.0.1";
	...
	public FetionClient(String account, String password,
			...
			TransferFactory transferFactory, FetionStore fetionStore,
			FetionTimer timer, FetionExecutor executor) {
		this(new User(account, password, "fetion.com.cn"), notifyEventListener,
				transferFactory, fetionStore, timer, executor);
	}
	 ...
	 * 
	 * @param user
	 *            登录用户，每一个客户端都只有一个唯一的登录用户对象
	 * @param notifyListener
	 *            通知监听器，需实现NotifyEventListener接口，用户设置的通知监听器，处理客户端主动发起的事件
	 * @param transferFactory
	 *            连接工厂，需实现TransferFactory接口，用于创建连接对象
	 * @param fetionStore
	 *            飞信存储对象，需实现FetionStore接口，存储了飞信好友列表等信息
	 * @param fetionTimer
	 *            飞信定时器，需实现FetionTimer接口，用于完成不同的定时任务
	 * @param fetionExecutor
	 *            飞信执行器，需实现FetionExecutor接口，可以在另外一个单独的线程中执行任务
	 */
	public FetionClient(User user, NotifyEventListener notifyEventListener,
			TransferFactory transferFactory, FetionStore fetionStore,
			FetionTimer fetionTimer, FetionExecutor fetionExecutor) {
		FetionConfig.init();
	...
	private void ensureOnline() {
		if (this.state != ClientState.ONLINE) {
			throw new IllegalStateException("client is not online [state="
					+ this.state + "], action failed.");
		}
	}
	...
	private void ensureSSISigned() {
		if (this.getFetionUser().getSsiCredential() == null) {
			throw new IllegalStateException(
					"Empty ssic, please enable ssi login first.");
		}
	}

	/**
	 * 更新客户端状态 这个方法是不同步的，因为属性state是volatile的
	 */
	@Override
	...
	/**
	 * 进行图片验证操作 一般的操作流程是在收到ImageVerifyEvent事件后，调用者提示用户识别后，
	...
	public void processVerify(ImageVerifyEvent event) {
		...
		// if(h.getName().equals(SipcHeader.AUTHORIZATION) &&
		// h.getValue().indexOf("Verify response=")!=-1) {
		// it.remove();
		// }
		// }
		//
		// request.addHeader(SipcHeader.AUTHORIZATION,
		// "Verify response=\""+image.getVerifyCode()+"\",algorithm=\""+image.getAlgorithm()+"\","
		// +
		...
		// }else {
		// throw new
		// IllegalArgumentException("Invalid verify action:"+event.getVerifyAction());
		// }
	}
	...
	public void cancelVerify(ImageVerifyEvent event) {
		if (this.state == ClientState.LOGGING) {
			...
			// if(listener!=null) {
			// listener.fireEevent(new
			// FailureEvent(FailureType.VERIFY_CANCELED));
			// }
		}
	}

	/**
	 * 处理客户端异常 通常交给客户端处理的异常都是致命的，也就是说如果调用了这个方法客户端都会主动退出
	 */
	public void handleException(FetionException exception) {
		...
		// }else
		// if(re.getRegistrationType()==RegistrationException.DISCONNECTED) {
		...
		// //系统错误,为了保证系统的稳定性，这里只是做个记录，并不退出客户端
		// logger.fatal("System error, just log it and ignore it. Not exit the client for stablity reason.",
		// exception);
		// CrushBuilder.handleCrushReport(exception, ((SystemException)
		// exception).getArgs()); //生成错误报告
		// return ; //返回，不退出客户端
		...
		// }else {
		// logger.fatal("Unknown error, just log it and ignore it. Not exit the client for stablity reason.",
		// exception);
	 ...
	 * 
	 * @param event
	 *            图片验证事件，如果获取成功，将会把获取的验证码图片注入到事件中
	 * @throws IOException
	 * @throws JDOMException
	 * @throws ParseException
	 */
	public void flushVerifyImage(ImageVerifyEvent event) throws ParseException,
	 ...
	 *             如果对话框建立失败则抛出
	 */
	// public ChatDialogProxy getChatDialogProxy(Buddy buddy) throws
	// DialogException
	// {
	...
	public void login(int presence) {
		...
		if (state == ClientState.LOGGING || state == ClientState.ONLINE) {
			throw new IllegalStateException("Client is " + state.toString()
					+ ", could not login again.");
		}
	...
	/**
	 * 客户端异步登陆登录 登陆结果在NotifyEventListener监听LoginStateEvent事件
	 */
	public void login() {
	...
	/**
	 * 取消当前登录操作，并释放资源 只能在登录的过程调用，否则会抛出IllegalStateException
	 */
	public void cancelLogin() {
		if (this.state == ClientState.LOGGING) {
			...
			throw new IllegalStateException(
					"client is not in logging state(state="
							+ state.toString()
							+ "). if client is online, please call FetionClient.logout().");
		}
	}
	 ...
	 * 
	 * @param toBuddy
	 *            发送聊天消息的好友，关系只能是好友或者陌生人才能发送消息，否则会得到FailureType.
	 *            BUDDY_RELATION_FORBIDDEN的错误
	 * @param message
	 *            需发送的消息
	 * @param listener
	 *            操作结果监听器
	 */
	// TODO 完成发送消息函数
	// public void sendChatMessage(final Buddy toBuddy, final Message message,
	...
	// if(listener!=null) listener.fireEevent(new
	// FailureEvent(FailureType.BUDDY_RELATION_FORBIDDEN));
	// }else {
	// try {
	...
	// this.sendChatMessage(toBuddy, message, new
	// FutureActionEventListener(future));
	// return future;
	// }
	 ...
	 */
	// TODO 完成发送消息函数
	// public void sendSMSMessage(Buddy toBuddy, Message message,
	// ActionEventListener listener)
	// {
	// this.ensureOnline();
	// if(toBuddy.getRelation()==Relation.BANNED) {
	// if(listener!=null) listener.fireEevent(new
	// FailureEvent(FailureType.BUDDY_BLOCKED));
	// }else {
	// this.dialogFactory.getServerDialog().sendSMSMessage(toBuddy, message,
	// listener);
	// }
	// }
	 ...
	 */
	// TODO 完成发送消息函数
	// public void sendSMSMessageToSelf(Message message, ActionEventListener
	// listener)
	// {
	// this.ensureOnline();
	 ...
	 */
	// TODO 完成发送消息函数
	// public void sendChatMessage(long mobile,final Message message, final
	// ActionEventListener listener)
	// {
	// this.ensureOnline();
	//
	 ...
	 */
	// TODO 完成发送消息函数
	// public void sendSMSMessage(long mobile, final Message message, final
	// ActionEventListener listener)
	// {
	// this.ensureOnline();
	//
	...
	// {
	// this.ensureOnline();
	// this.dialogFactory.getServerDialog().setBuddyLocalName(buddy, localName,
	// listener);
	// }

	/**
	 * 移动好友到多个分组 飞信好友分组有点奇怪，一个好友可以属于多个分组，谁想出的这个需求····
	 * 
	 * @param buddy
	 *            好友对象
	 * @param cordIds
	...
	// {
	// this.ensureOnline();
	// this.dialogFactory.getServerDialog().setBuddyCord(buddy, cordList,
	// listener);
	// }
	 ...
	 * @param listener
	 */
	// public void setBuddyCord(Buddy buddy, Cord cord, ActionEventListener
	// listener)
	// {
	// this.ensureOnline();
	 ...
	 *            手机号码或者飞信号码
	 * @param localName
	 *            添加好友后显示的本地名称，如果为null,默认显示好友的昵称
	 * @param cord
	 *            好友添加到分组的编号，如果为null，添加到默认分组
	 * @param desc
	 *            对自己的说明 ，如 xxx，在对方飞信好友里就会收到：我是xxxx
	 * @param promptId
	 *            提示的信息编号,见下面的说明。如果不清楚，请直接传递0
	 * @param localName
	 * @param listener
	 *            结果监听器
	 * 
	 ...
	 * </pre>
	 */
	// public void addBuddy(String account, String localName, Cord cord, String
	// desc, int promptId, ActionEventListener listener)
	// {
	// this.ensureOnline();
	// AccountValidator validator = new AccountValidator(account);
	// if(validator.isValidMobile()){
	// this.dialogFactory.getServerDialog().addBuddy("tel:"+account, localName,
	// cord, desc, promptId, listener);
	// }else if(validator.getFetionId()>0){
	// this.dialogFactory.getServerDialog().addBuddy("sip:"+account, localName,
	// cord, desc, promptId, listener);
	// }else{
	// if(listener!=null){
	// listener.fireEevent(new FailureEvent(FailureType.INVALID_ACCOUNT));
	...
	// {
	// this.ensureOnline();
	// this.addBuddy(account, null, null, this.user.getDisplayName(), 0,
	// listener);
	// }
	 ...
	 * @param listener
	 */
	// public void declinedApplication(Buddy buddy, ActionEventListener
	// listener)
	// {
	// this.ensureOnline();
	...
	// }else{
	// if(listener!=null) listener.fireEevent(new
	// FailureEvent(FailureType.INVALID_PRESENCE_VALUE));
	// }
	// }
	 ...
	 * @param listener
	 */
	// public void setCordTitle(Cord cord, String title, ActionEventListener
	// listener)
	// {
	// this.ensureOnline();
	...
	/**
	 * 获取定时短信列表 注意：在登录的过程中并没有获取定时短信这一步，所以在这里需要手动的进行获取短信列表
	 * 
	 * @param listener
	 */
	 ...
	 *            定时短信内容
	 * @param sendDate
	 *            发送时间 ，有效时间是在当前时间后11分钟到一年以内，否则会返回错误
	 * @param receiverList
	 *            短信的接收者列表
	 * @param listener
	 *            操作监听器
	 */
	// public void createScheduleSMS(Message message, Date sendDate,
	// Collection<Buddy> receiverList, ActionEventListener listener)
	// {
	// this.ensureOnline();
	// //检查定时短信的定时时间，最短时间是 当前时间+11分钟-一年后当前时间
	// //比如当前时间是 2007.7.1 22:56 有效的时间是 2010.7.1 23:07 - 2011.7.1
	// 22:56，在这个时间之内的才是有效时间，
	// Calendar calMin = Calendar.getInstance();
	// calMin.add(Calendar.MINUTE, 11);
	// Calendar calMax = Calendar.getInstance();
	// calMax.add(Calendar.YEAR, 1);
	// //判断是否在有效的范围内，如果不在这个范围内则返回发送时间错误
	// if( sendDate.after(calMin.getTime()) &&
	// sendDate.before(calMax.getTime())){
	// ScheduleSMS sc = new ScheduleSMS(-1, message, sendDate, receiverList);
	// this.getServerDialog().createScheduleSMS(sc, listener);
	// }else{
	// if(listener!=null) listener.fireEevent(new
	// FailureEvent(FailureType.INVALID_SEND_DATE));
	// }
	// }
	...
	/**
	 * 将好友添加到黑名单，也就是无法发送消息给在黑名单的好友 添加之后和该好友的关系变为Relation.BANNED.
	 * 
	 * @param buddy
	 *            需添加到黑名单的好友
	 * @param listener
	 */
	// public void addBuddyToBlackList(Buddy buddy, ActionEventListener
	// listener)
	// {
	// this.ensureOnline();
	// if(buddy.getRelation()==Relation.BANNED) {
	// if(listener!=null) listener.fireEevent(new
	// FailureEvent(FailureType.BUDDY_IN_BLACKLIST));
	// }else {
	...
	/**
	 * 将好友从黑名单中删除 删除好友的关系变为Relation.BUDDY
	 * 
	 * @param buddy
	 *            需从黑名单中删除的好友
	 * @param listener
	 */
	// public void removeBuddyFromBlackList(Buddy buddy, ActionEventListener
	// listener)
	// {
	// this.ensureOnline();
	// if(buddy.getRelation()!=Relation.BANNED) {
	// if(listener!=null) listener.fireEevent(new
	// FailureEvent(FailureType.BUDDY_NOT_IN_BLACKLIST));
	// }else {
	...
	// throw new
	// IllegalArgumentException("portrait height and width should below 120 px.");
	// }else {
