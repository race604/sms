 ...
 * 
 * 
 * 抽象的多可以提供多个独立连接的工厂 实现了建立默认连接等方法
 * 
 * @author solosky <solosky772@qq.com>
 * 
 */
public abstract class MutiConnectionTransferFactory implements TransferFactory {
	...
	public Transfer createDefaultTransfer() throws TransferException {
		...
		String sipcProxy = this.context.getLocaleSetting().getNodeText(
				"/config/servers/sipc-proxy");
		String sipcSslProxy = this.context.getLocaleSetting().getNodeText(
				"/config/servers/sipc-ssl-proxy");

		if (sipcProxy == null)
			sipcProxy = FetionConfig.getString("server.sipc-proxy");
		if (sipcSslProxy == null)
			sipcSslProxy = FetionConfig.getString("server.sipc-ssl-proxy");
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.net.TransferFactory#getDefaultTransferLocalPort()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.net.TransferFactory#isMutiConnectionSupported()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * net.solosky.maplefetion.net.TransferFactory#setFetionContext(net.solosky
	 * .maplefetion.FetionContext)
	 */
	@Override
