...
public class MainActivity extends SherlockListActivity implements
		OnGesturePerformedListener, OnItemClickListener {
	...
	private boolean showAllMenu(Menu menu) {
		...
		menu.add(0, R.string.option, 0, R.string.option)
	...
	public void onGesturePerformed(GestureOverlayView overlay, Gesture gesture) {
		...
		if (predictions.size() > 0 && predictions.get(0).score > 1.0) {
			...
			MainActivityAdapter.ViewHolder holder = (MainActivityAdapter.ViewHolder) mCurrentView
					.getTag();
			...
			if ("right".equals(action)) {
				Toast.makeText(this, "Delet the SMS: " + holder.body.getText(),
						Toast.LENGTH_SHORT).show();
			} else if ("left".equals(action)) {
				Toast.makeText(this, "Left: " + holder.body.getText(),
						Toast.LENGTH_SHORT).show();
			} else if ("up".equals(action)) {
				Toast.makeText(this, "Up: " + holder.body.getText(),
						Toast.LENGTH_SHORT).show();
			} else if ("down".equals(action)) {
				Toast.makeText(this, "Down: " + holder.body.getText(),
						Toast.LENGTH_SHORT).show();
			}
		}
	}
